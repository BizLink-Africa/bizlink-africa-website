...
import { reauthenticateForPayouts } from '@/app/admin/(protected)/payouts/reauth-actions';

export default function PayoutReauthPrompt() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    setError(null);
    const result = await reauthenticateForPayouts(password);
    setSubmitting(false);
    if (!result.success) {
  ...
  return (
    <div className="max-w-sm mx-auto py-16 text-center">
      ...
      <p className="text-sm text-[#707975] mb-6">
        Approving a payout or submitting one to the Selcom sandbox requires re-entering your password — this expires
        after 10 minutes. SANDBOX — NO LIVE FUNDS: submissions call Selcom&apos;s real sandbox API, but can never
        reach their production environment.
      </p>
