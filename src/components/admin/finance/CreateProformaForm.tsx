...
import { createProforma, type ProformaInput } from '@/app/admin/(protected)/finance/proformas/actions';
...
export default function CreateProformaForm({ defaults }: { defaults: Defaults }) {
  ...
  validUntilDate.setDate(validUntilDate.getDate() + defaults.paymentTermsDays);
  const validUntilDefault = validUntilDate.toISOString().slice(0, 10);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await createProforma(form as ProformaInput);
    setSubmitting(false);

    if (result.success && result.id) {
      router.push(`/admin/finance/proformas/${result.id}`);
    } else {
      setError(result.message ?? 'Failed to create proforma invoice.');
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-[#bfc9c4] p-5 space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-[#00342b]">New Proforma Invoice</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="sm:col-span-2">
          ...
          <input id="serviceSummary" value={form.serviceSummary} onChange={handleText('serviceSummary')} placeholder="e.g. AI Sales Agent + WhatsApp Business API setup" className={inputClass} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <input id="validUntil" type="date" value={form.validUntil} onChange={handleText('validUntil')} className={inputClass} />
        </div>
        <div>
          <label className={labelClass} htmlFor="paymentTerms">Payment Terms</label>
          <input id="paymentTerms" value={form.paymentTerms} onChange={handleText('paymentTerms')} className={inputClass} />
        </div>
      </div>
      ...
      <div>
        <label className={labelClass} htmlFor="termsAndConditions">Terms &amp; Conditions</label>
        <textarea id="termsAndConditions" rows={2} value={form.termsAndConditions} onChange={handleText('termsAndConditions')} className={`${inputClass} resize-none`} />
      </div>
