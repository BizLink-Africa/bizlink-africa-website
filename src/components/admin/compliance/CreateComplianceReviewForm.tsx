...
import { COMPLIANCE_CATEGORIES, RISK_LEVELS } from '@/data/compliance';
import { createComplianceReview, type ComplianceReviewInput } from '@/app/admin/(protected)/compliance/reviews/actions';

const initialForm = {
  title: '',
  category: COMPLIANCE_CATEGORIES[0].value,
  department: '',
  reviewer: '',
  startDate: '',
  dueDate: '',
...
export default function CreateComplianceReviewForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await createComplianceReview({
      ...form,
      riskLevel: (form.riskLevel || undefined) as ComplianceReviewInput['riskLevel'],
    } as ComplianceReviewInput);
    setSubmitting(false);
    ...
    if (result.success) {
      ...
      setError(result.message ?? 'Failed to create compliance review.');
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-[#bfc9c4] p-5 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-[#00342b]">New Compliance Review</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <select id="category" name="category" value={form.category} onChange={handleChange} className={inputClass}>
            {COMPLIANCE_CATEGORIES.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
          </select>
        </div>
        <div>
          <label className={labelClass} htmlFor="department">Department (if not client-specific)</label>
          <input id="department" name="department" value={form.department} onChange={handleChange} className={inputClass} />
        </div>
        <div>
          <label className={labelClass} htmlFor="reviewer">Officer / Reviewer</label>
          <input id="reviewer" name="reviewer" value={form.reviewer} onChange={handleChange} className={inputClass} />
        </div>
        <div>
          <label className={labelClass} htmlFor="riskLevel">Risk</label>
          <select id="riskLevel" name="riskLevel" value={form.riskLevel} onChange={handleChange} className={inputClass}>
