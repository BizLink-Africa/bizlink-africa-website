...
export default function ClientFilters({
  ...
  onboardingStatuses: readonly Option[];
  integrationStatuses: readonly Option[];
}) {
    ...
    <div className="bg-white border border-[#bfc9c4] p-4 flex flex-wrap gap-3 items-end">
      <form onSubmit={handleSearchSubmit} className="flex-1 min-w-[220px]">
        ...
        <input
          ...
          placeholder="Client, business, email, phone, or location"
          className="w-full border border-[#bfc9c4] px-3 py-2 text-sm focus:border-[#00342b] focus:outline-none"
        />
      </form>

      <div>
        <label className="block text-xs font-semibold text-[#707975] uppercase tracking-wider mb-1">Onboarding</label>
        <select
          defaultValue={searchParams.get('onboarding') ?? ''}
          onChange={(e) => updateParam('onboarding', e.target.value)}
      ...
      <div>
        <label className="block text-xs font-semibold text-[#707975] uppercase tracking-wider mb-1">Integration</label>
        <select
          defaultValue={searchParams.get('integration') ?? ''}
      ...
      <div>
        <label className="block text-xs font-semibold text-[#707975] uppercase tracking-wider mb-1">Active</label>
        <select
