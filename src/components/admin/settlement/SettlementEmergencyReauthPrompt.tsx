...
import { reauthenticateForSettlementEmergency } from '@/app/admin/(protected)/settlement/reauth-actions';

export default function SettlementEmergencyReauthPrompt() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    setError(null);
    const result = await reauthenticateForSettlementEmergency(password);
    setSubmitting(false);
    if (!result.success) {
  ...
  return (
    <div className="max-w-sm mx-auto py-16 text-center">
      ...
      <p className="text-sm text-[#707975] mb-6">
        This batch is eligible for a Super Admin emergency override, which requires re-entering your password — this expires after 10 minutes.
      </p>
