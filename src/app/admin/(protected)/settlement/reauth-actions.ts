...
import { SETTLEMENT_EMERGENCY_REAUTH_PURPOSE, REAUTH_TTL_MINUTES } from '@/lib/supabase/reauth';
import { assertMerchantSettlementsNotBizLinkManaged } from '@/lib/archived-financial-prototype';

export async function reauthenticateForSettlementEmergency(password: string): Promise<{ success: boolean; message?: string }> {
  try {
  ...
  const { error: insertError } = await supabase.from('staff_reauth_sessions').insert({
    user_id: user.id,
    purpose: SETTLEMENT_EMERGENCY_REAUTH_PURPOSE,
    expires_at: expiresAt,
  });

  if (insertError) {
    console.error('Failed to record settlement emergency reauth session', insertError);
